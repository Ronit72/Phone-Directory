...
bool checkno(char nos[100])
{
	...
	if(c<10)
	{
		printf("\tLENGTH OF NUMBER IS LESS THAN 10!!!\n");
		printf("\tENTER AGAIN!!!\n");
		return 0;
	}
	for(i=0;i<c;i++)
	{
		if(nos[i]>='a'&&nos[i]<='z'||nos[i]>='A'&&nos[i]<='Z'||nos[i]==' '||c<10)
		{
...
void delfile(char name[100])
{
	...
	fclose(fp1);
	fclose(fp);
	remove("phonedirec.dat");
	rename("temp.dat", "phonedirec.dat");
}
void searchdelfile(char name2[100])
{
	...
	while(!feof(fp))
	{
		...
		{
		    x=1;
		    printf("\n\t_________DATA FOUND IN FILE__________\n\n");
		    fclose(fp);
		    delfile(name1);
			printf("%s is deleted successfully\n",name1);
            printf("\n\t_________Data deleted in file________\n");
            break;
        }
	}
	if(x==0)
    {
        printf("\n\t_________DATA NOT FOUND__________\n\n");
    }
}
	...
	while(!feof(fp))
	{
		...
		if(!strcmp(n,name1))
		{
			...
			if(mod==1)
			{
                ...
                goto poi;
                modifyfile(n,str);
				printf("\n\t_________Data Updated in file________\n");
                break;
			}
	    }
	}
...
void search(char n[100])
{
	...
	while(cur!=NULL)
	{
		if(strcmp(cur->name,n)==0)
		{
			...
			if(mod==1)
			{
                ...
                modifyfile(n,str);
				printf("\n\t_________Data Updated in file and linked list________\n");
	...
	printf("\n\t_______DATA NOT FOUND IN LINKED LIST_______\n\n");
	searchfile(n);
}
